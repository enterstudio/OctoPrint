...
.. http:get:: /api/slicing/profiles

   Returns a list of all available slicing profiles for all registered slicers in the system.

   Returns a :http:statuscode:`200` response with a :ref:`List All Profiles Response <sec-api-slicing-datamodel-listall>`
...
.. http:get:: /api/slicing/(string:slicer)/profiles
   ...
   Returns a :http:statuscode:`200` response with a :ref:`List Profiles Response <sec-api-slicing-datamodel-list>`
   as the body upon successful completion.
   ...
   **Example**
   ...
   :param slicer:   The id of the slicer for which to list the available profiles.
   :statuscode 200: No error
...
.. http:get:: /api/slicing/(string:slicer)/profiles/(string:name)
...
.. http:put:: /api/slicing/(string:slicer)/profiles/(string:name)

   Adds a new slicing profile for the given ``slicer`` to the system. If the profile named ``name`` already exists,
   ...
   **Example**
   ...
   :param slicer:   The name of the slicer for which to add the profile
...
.. http:delete:: /api/slicing/(string:slicer)/profiles/(string:name)

   Delete the slicing profile named ``name`` for the slicer ``slicer``.

   :param slicer:   The name of the slicer for which to delete the profile
...
.. _sec-api-slicing-datamodel-listall:
...
.. list-table::
   ...
   * - ``<slicer name>``
     - 0..1
     - :ref:`List Response <sec-api-slicing-datamodel-list>`
     - All profiles stored for the slicer.

.. _sec-api-slicing-datamodel-list:
