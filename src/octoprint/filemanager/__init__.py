...
from .analysis import QueueEntry
...
class FileManager(object):
	def __init__(self, analysis_queue, slicing_manager):
		self._analysis_queue = analysis_queue
		self._analysis_queue.register_finish_callback(self._on_analysis_finished)

		self._storage_managers = dict()
		self._storage_managers[FileDestinations.LOCAL] = LocalFileStorage(settings().getBaseFolder("uploads"))
	...
	def add_storage(self, type, storage_manager):
		self._storage_managers[type] = storage_manager
	...
	def add_folder(self, destination, path, ignore_existing=True):
		return self._storage(destination).add_folder(path, ignore_existing=ignore_existing)
