...
def uploadGcodeFile(target):
	...
	slicer_name = request.values["slicer"] if "slicer" in request.values and request.values["slicer"] in fileManager.registered_slicers else ("cura" if "cura" in fileManager.registered_slicers else None)
	...
	try:
		...
		futureFilename = None
	if futureFilename is None or (not settings().getBoolean(["cura", "enabled"]) and not octoprint.filemanager.valid_file_type(futureFilename, type="gcode")):
	...
	if octoprint.filemanager.valid_file_type(added_file, "stl") and slicer_name:
		...
		gcode_path = name + ".gco"
		gcode_absolute_path = fileManager.get_absolute_path(FileDestinations.LOCAL, gcode_path)
		ok, result = fileManager.slice(slicer_name, FileDestinations.LOCAL, added_file, FileDestinations.LOCAL, gcode_path, callback=fileProcessingFinished, callback_args=(gcode_path, gcode_absolute_path, target))
		if not ok:
			return make_response("Could not slice {added_file}: {result}".format(**locals()))
		filename = added_file
		done = False
	else:
